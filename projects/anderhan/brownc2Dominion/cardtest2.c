...
int main() {
	...
	int shuffledCards = 0;
	int result = 0;
	int add_actions = 0, action_taken = 0;
	...
	/************************** TEST 1 ***************************/
	printf("Test 1: Current player should receive 1 card\n");
	...
	//one card should be added to the hand
	newCards = 1;
	...
	if (result == 1){
		printf("	The number cards in hand did not increase by 1\n");
		printf("FAILED\n\n");
	}
	...
	/**************************** TEST 2 **************************/
	printf("Test 2: Current deck should decrease by 1 card\n");
	...
	result = pass_or_fail(testG.deckCount[thisPlayer], 
								 G.deckCount[thisPlayer] - newCards + shuffledCards);
	if (result == 1){
		printf("	The deck did not decrease by 1\n");
		printf("FAILED\n\n");
	}
	...
	printf("	Actions = %d, expected = %d\n", testG.numActions, 
														  G.numActions - action_taken + add_actions);
	result = pass_or_fail(testG.numActions, G.numActions - action_taken + add_actions);
	if (result == 1){
